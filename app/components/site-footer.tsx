...
export function SiteFooter() {
  ...
  return (
    <footer className="site-footer">
      ...
      <div className="container footer-bottom"><span>(c) {new Date().getFullYear()} {config.brand.name}</span><span><Link href="/privacy">{config.content.legal.privacyLabel}</Link> - <Link href="/terms">{config.content.legal.termsLabel}</Link> - <Link href="/accessibility">{config.content.legal.accessibilityLabel}</Link></span></div>
    </footer>
  );
}
