...
export function CheckoutForm({ onComplete }: { onComplete: () => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    trackAnalytics("checkout_started", { payload: { itemCount: cart.length } });
    void fetch("/api/checkout/abandon", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: values.email, cart, subtotal: cart.reduce((sum, item) => sum + item.price * item.quantity, 0), currency: config.commerce.currency }) }).catch(() => undefined);
