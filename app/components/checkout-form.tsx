...
type SavedAddress = Partial<CheckoutValues> & { id: string; label?: string; isDefault?: boolean };
...
export function CheckoutForm({ onComplete, onQuoteChange }: { onComplete: () => void; onQuoteChange?: (quote: CheckoutQuote | null) => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const addressPayload = await addressResponse.json().catch(() => ({})) as { addresses?: SavedAddress[] };
  ...
  const deliveryComplete = ["firstName", "lastName", "address", "city", "region", "zip"].every((field) => values[field as keyof CheckoutValues].trim());
  const checkoutStep = !values.email.trim() ? 1 : deliveryComplete ? 3 : 2;
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    trackAnalytics("paypal_redirect_started", { payload: { total: quote.total } });
    ...
    }).catch((error) => { setSubmitting(false); const message = error instanceof Error ? error.message : "Unable to start payment."; setServerError(message); trackAnalytics("paypal_redirect_failed", { payload: { message } }); showToast(message, "error"); });
  }

  return <form className="checkout-form" onSubmit={submit} noValidate aria-busy={submitting || quotePending}>
    ...
    <ol className="checkout-steps" aria-label="Checkout progress"><li className={checkoutStep > 1 ? "is-complete" : checkoutStep === 1 ? "is-active" : ""} aria-current={checkoutStep === 1 ? "step" : undefined}><span>1</span>Contact</li><li className={checkoutStep > 2 ? "is-complete" : checkoutStep === 2 ? "is-active" : ""} aria-current={checkoutStep === 2 ? "step" : undefined}><span>2</span>Delivery</li><li className={checkoutStep === 3 ? "is-active" : ""} aria-current={checkoutStep === 3 ? "step" : undefined}><span>3</span>PayPal</li></ol>
    {serverError && <div className="form-error checkout-form-alert" role="alert">{serverError}<button type="button" className="text-button" onClick={() => setServerError("")}>Dismiss</button></div>}
    <fieldset><legend>Contact</legend><Field id="checkout-email" label="Email address" value={values.email} error={errors.email} type="email" autoComplete="email" placeholder="you@example.com" onChange={(value) => update("email", value)} /><label className="checkbox-label"><input type="checkbox" checked={newsletterOptIn} onChange={(event) => setNewsletterOptIn(event.target.checked)} /> Email me with field notes and new gear</label></fieldset>
    <fieldset><legend>Delivery</legend>{savedAddresses.length > 0 && <label className="checkout-field" htmlFor="checkout-saved-address"><span>Saved address</span><select id="checkout-saved-address" defaultValue="" onChange={(event) => applySavedAddress(event.target.value)}><option value="">Choose a saved address</option>{savedAddresses.map((address) => <option key={address.id} value={address.id}>{address.label || `${address.firstName || ""} ${address.lastName || ""}`.trim() || "Saved address"}{address.isDefault ? " (default)" : ""}</option>)}</select></label>}<div className="form-two"><Field id="checkout-first-name" label="First name" value={values.firstName} error={errors.firstName} placeholder="First name" autoComplete="given-name" onChange={(value) => update("firstName", value)} /><Field id="checkout-last-name" label="Last name" value={values.lastName} error={errors.lastName} placeholder="Last name" autoComplete="family-name" onChange={(value) => update("lastName", value)} /></div><Field id="checkout-address" label="Address" value={values.address} error={errors.address} placeholder="Street address" autoComplete="street-address" onChange={(value) => update("address", value)} /><div className="form-three"><Field id="checkout-city" label="City" value={values.city} error={errors.city} placeholder="City" autoComplete="address-level2" onChange={(value) => update("city", value)} /><Field id="checkout-region" label="State / region" value={values.region} error={errors.region} placeholder="State" autoComplete="address-level1" onChange={(value) => update("region", value)} /><Field id="checkout-zip" label="ZIP code" value={values.zip} error={errors.zip} placeholder="ZIP" autoComplete="postal-code" onChange={(value) => update("zip", value)} /></div><SelectField id="checkout-country" label="Country" value={values.country} options={["United States", "Canada", "United Kingdom", "Australia"]} onChange={(value) => update("country", value)} /><SelectField id="checkout-delivery" label="Delivery method" value={values.deliveryMethod} options={["Standard delivery", "Express delivery"]} onChange={(value) => update("deliveryMethod", value)} /><Field id="checkout-coupon" label="Coupon code (optional)" value={values.couponCode} placeholder="WELCOME10" autoComplete="off" onChange={(value) => update("couponCode", value.toUpperCase())} />{quotePending && <p className="quote-status" role="status">Updating your order total...</p>}{quoteError && <p className="quote-status is-error" role="alert">{quoteError}</p>}{quote && values.couponCode && <p className={`quote-status ${quote.couponApplied ? "is-success" : "is-error"}`}>{quote.couponApplied ? `Coupon ${quote.couponCode} applied.` : "That coupon is not valid for this order."}</p>}{quote && quote.freeShippingThreshold > 0 && <p className="quote-status">{quote.shipping === 0 ? "Free shipping unlocked." : `Add ${formatMoney(quote.freeShippingThreshold - quote.subtotal + quote.discount, config.commerce.currency)} more to unlock free shipping.`}</p>}</fieldset>
...
function Field({ id, label, value, error, placeholder, type = "text", autoComplete, onChange }: { id: string; label: string; value: string; error?: string; placeholder: string; type?: string; autoComplete?: string; onChange: (value: string) => void }) {
  return <label className={`checkout-field ${error ? "has-error" : ""}`} htmlFor={id}><span>{label}</span><input id={id} type={type} value={value} placeholder={placeholder} autoComplete={autoComplete} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} onChange={(event) => onChange(event.target.value)} />{error && <span className="field-error" id={`${id}-error`}>{error}</span>}</label>;
}
