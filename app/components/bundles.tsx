...
export function BundleStrip({ catalog }: { catalog: Product[] }) {
  const { activeSiteId, site, config } = useSiteRuntime(); const { addToCart } = useStore(site?.id || activeSiteId); const [bundles, setBundles] = useState<Bundle[]>([]);
