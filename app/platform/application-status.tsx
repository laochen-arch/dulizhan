...
const statusLabels: Record<string, string> = { draft: "Draft", submitted: "Submitted", reviewing: "In review", needs_info: "Action required", approved: "Approved", rejected: "Not approved", site_created: "Storefront ready" };
const statusOrder = ["draft", "submitted", "reviewing", "approved", "site_created"];

function statusLabel(status: string) { return statusLabels[status] || status.replaceAll("_", " "); }
function progressIndex(status: string) { if (status === "needs_info" || status === "rejected") return 1; const index = statusOrder.indexOf(status); return index < 0 ? 0 : index; }
...
export function PlatformApplicationStatus() {
  ...
  const checklist = [
    ...
    { label: "Domain request", done: detail.domains.some((item) => ["pending", "reviewing", "active"].includes(item.status)), detail: detail.domains[0]?.hostname || "Optional until the storefront is approved." },
  ];
