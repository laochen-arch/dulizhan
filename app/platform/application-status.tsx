...
export function PlatformApplicationStatus() {
  ...
  const loadDetail = useCallback(async (id: string, accessToken = "") => {
    ...
    setDetail(payload);
    setSelectedId(id);
    setAssetEdits(Object.fromEntries(payload.assets.map((asset) => [asset.id, { assetKey: asset.assetKey, kind: asset.kind }] )));
  ...
  async function saveAssetBinding(assetId: string) {
    ...
    if (!edit?.assetKey.trim()) return;
    setBusy(true); setNotice("");
    try {
      const response = await fetch(`/api/platform/applications/assets/${encodeURIComponent(assetId)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ applicationId: detail.application.id, token, ...edit }) });
      const payload = await response.json().catch(() => ({})) as { error?: string };
