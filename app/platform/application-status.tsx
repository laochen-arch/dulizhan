...
type Notification = { id: string; eventType: string; subject: string; status: string; attempts: number; lastError: string | null; sentAt: string | null; createdAt: string; updatedAt: string };
type Detail = { application: Application; events: Event[]; domains: DomainRequest[]; assets: Asset[]; tickets: Ticket[]; notifications: Notification[]; commercial: PlatformCommercialSnapshot | null; canReview: boolean };
...
const statusOrder = ["draft", "submitted", "reviewing", "approved", "site_created", "live"];

function statusLabel(status: string) { return statusLabels[status] || status.replaceAll("_", " "); }
function progressIndex(status: string) { if (status === "needs_info" || status === "rejected") return 1; if (status === "commercial_pending") return 3; if (status === "site_creating" || status === "onboarding_failed") return 4; const index = statusOrder.indexOf(status); return index < 0 ? 0 : index; }
...
export function PlatformApplicationStatus() {
  ...
  const [assetEdits, setAssetEdits] = useState<Record<string, { assetKey: string; kind: string }>>({});
  ...
  const loadDetail = useCallback(async (id: string, accessToken = "") => {
    const params = new URLSearchParams({ id }); if (accessToken) params.set("token", accessToken);
    ...
    setDetail(payload); setSelectedId(id); setAssetEdits(Object.fromEntries(payload.assets.map((asset) => [asset.id, { assetKey: asset.assetKey, kind: asset.kind }])));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const queryId = query.get("application") || query.get("id"); const queryToken = query.get("token") || "";
      setToken(queryToken); if (queryId) await loadDetail(queryId, queryToken); else await loadList();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load application status."); }
  ...
  async function reloadDetail() { if (selectedId) await loadDetail(selectedId, token); else await refresh(); }

  async function submitSupplement(event: React.FormEvent) {
    event.preventDefault(); if (!detail) return; setBusy(true); setNotice(""); setError("");
    try {
      const response = await fetch("/api/platform/applications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: detail.application.id, token, ...supplement, draft: false, agreementAccepted: Boolean(detail.application.agreementAcceptedAt), agreementVersion: detail.application.agreementVersion || "platform-v1" }) });
      ...
      setNotice("Your updates were sent back to the platform team."); await reloadDetail();
  ...
  async function requestDomain(event: React.FormEvent) {
    event.preventDefault(); if (!detail || !domain.trim()) return; setBusy(true); setNotice(""); setError("");
    try {
  ...
  async function uploadAssets(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const files = Array.from(event.target.files); setBusy(true); setNotice(""); setError("");
    ...
    for (const file of files) {
      ...
      try {
        ...
        const response = await fetch("/api/platform/applications/assets", { method: "POST", body }); const payload = await response.json().catch(() => ({})) as { error?: string };
      ...
      } catch (cause) { failures.push(`${file.name}: ${cause instanceof Error ? cause.message : "upload failed"}`); setUploadStatus((current) => ({ ...current, [file.name]: "Failed — retry this file" })); }
    }
    event.target.value = "";
    if (failures.length) setError(failures.join("; ")); else setNotice(`${files.length} image(s) uploaded to the application workspace.`);
    try { await reloadDetail(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to refresh uploaded images."); }
    finally { setBusy(false); }
  }

  async function saveAssetBinding(assetId: string) {
    if (!detail) return; const edit = assetEdits[assetId]; if (!edit?.assetKey.trim()) return; setBusy(true); setNotice(""); setError("");
    try {
      const response = await fetch(`/api/platform/applications/assets/${encodeURIComponent(assetId)}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ applicationId: detail.application.id, token, ...edit }) }); const payload = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Unable to save the media binding.");
      setNotice("Media binding saved. It will be used when the storefront draft is initialized."); await reloadDetail();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to save the media binding."); }
    finally { setBusy(false); }
  }

  async function retryNotification(notificationId: string) {
    if (!detail) return; setBusy(true); setNotice(""); setError("");
    try {
      const response = await fetch("/api/platform/applications/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ applicationId: detail.application.id, notificationId }) }); const payload = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Unable to retry the notification.");
      setNotice("Notification retry recorded."); await reloadDetail();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to retry the notification."); }
    finally { setBusy(false); }
  }
  ...
  async function resendAccessLink() {
    if (!detail) return; setBusy(true); setNotice(""); setError("");
    try {
      const response = await fetch("/api/platform/applications/access", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "resend_access_link", applicationId: detail.application.id, token }) });
      const payload = await response.json().catch(() => ({})) as { error?: string; statusUrl?: string };
      if (!response.ok || !payload.statusUrl) throw new Error(payload.error || "Unable to issue a secure access link.");
      window.location.assign(payload.statusUrl);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to issue a secure access link."); setBusy(false); }
  }
  ...
  const checklist = [
    { label: "Application submitted", done: application.status !== "draft", detail: application.status === "draft" ? "Save the first step to submit this application." : "Your business profile is on file." },
    { label: "Template selected", done: Boolean(application.templateSiteId), detail: application.templateSiteId || "Choose a starting template." },
    ...
    { label: "Domain verification", done: detail.domains.some((item) => item.status === "active"), detail: detail.domains[0]?.hostname || "Optional until the storefront is approved." },
  ];
  const resumeUrl = `/platform/apply?application=${encodeURIComponent(application.id)}${token ? `&token=${encodeURIComponent(token)}` : ""}`;
  ...
  return <div className="platform-application-workspace">
    ...
    <div className="platform-status-header"><div><p className="eyebrow">Application {application.id}</p><h2>{application.brandName}</h2><p>{application.companyName} · {application.email}</p></div><div className="v6-actions"><span className={`platform-status-badge ${application.status}`}>{statusLabel(application.status)}</span><button type="button" className="button button-outline" onClick={() => void resendAccessLink()} disabled={busy}>Email secure link</button></div></div>
    ...
    <div className="platform-status-grid"><section className="platform-status-panel"><div className="platform-panel-heading"><div><p className="eyebrow">Product & media handoff</p><h3>Keep launch materials together.</h3></div><span>{detail.assets.length} media</span></div><p className="v6-muted">Import CSV or JSON from the application form. Upload product, hero or brand images here, then set a binding key that matches the product image field or a homepage asset key.</p><div className="platform-upload-controls"><label className="v6-field"><span>Asset type</span><select value={assetKind} onChange={(event) => setAssetKind(event.target.value)}><option value="product">Product</option><option value="hero">Hero</option><option value="brand">Brand / logo</option><option value="general">General</option></select></label><label className="button button-outline platform-file-button"><span>Upload images</span><input type="file" accept="image/*" multiple onChange={(event) => void uploadAssets(event)} disabled={busy} /></label></div>{Object.entries(uploadStatus).length > 0 && <div className="platform-upload-status" role="status">{Object.entries(uploadStatus).map(([name, status]) => <span key={name}>{name}: {status}</span>)}</div>}<div className="platform-asset-grid">{detail.assets.map((asset) => { const edit = assetEdits[asset.id] || { assetKey: asset.assetKey, kind: asset.kind }; return <figure key={asset.id}><img src={asset.url} alt={asset.alt || asset.assetKey} /><figcaption><strong>{asset.assetKey}</strong><small>{asset.kind} · {Math.ceil(asset.sizeBytes / 1024)} KB</small><label className="platform-asset-binding"><span>Binding key</span><input value={edit.assetKey} onChange={(event) => setAssetEdits((current) => ({ ...current, [asset.id]: { ...edit, assetKey: event.target.value } }))} /></label><label className="platform-asset-binding"><span>Type</span><select value={edit.kind} onChange={(event) => setAssetEdits((current) => ({ ...current, [asset.id]: { ...edit, kind: event.target.value } }))}><option value="product">Product</option><option value="hero">Hero</option><option value="brand">Brand / logo</option><option value="general">General</option></select></label><button type="button" className="button button-outline" onClick={() => void saveAssetBinding(asset.id)} disabled={busy}>Save binding</button></figcaption></figure>; })}{!detail.assets.length && <div className="v6-empty">No uploaded images yet.</div>}</div></section><section className="platform-status-panel"><div className="platform-panel-heading"><div><p className="eyebrow">Domain request</p><h3>Connect the storefront address.</h3></div></div><form className="v6-form" onSubmit={requestDomain}><label className="v6-field"><span>Hostname</span><input required value={domain} onChange={(event) => setDomain(event.target.value)} placeholder="shop.example.com" /></label><button className="button button-dark" disabled={busy}>{busy ? "Submitting..." : "Request domain review →"}</button></form><div className="platform-domain-list">{detail.domains.map((item) => <div key={item.id}><strong>{item.hostname}</strong><span className={`platform-status-badge ${item.status}`}>{statusLabel(item.status)}</span><small>{item.note || "Awaiting platform review."}</small></div>)}{!detail.domains.length && <p className="v6-muted">No domain request yet. You can add one before approval.</p>}</div></section></div>
    <section className="platform-status-panel"><div className="platform-panel-heading"><div><p className="eyebrow">Status notifications</p><h3>Important changes are traceable.</h3></div><span>{detail.notifications.length}</span></div><div className="platform-notification-list">{detail.notifications.map((item) => <div key={item.id}><div><strong>{item.subject}</strong><small>{new Date(item.createdAt).toLocaleString()} · {item.attempts} attempt(s)</small></div><span className={`platform-status-badge ${item.status}`}>{item.status}</span>{item.lastError && <small>{item.lastError}</small>}{detail.canReview && item.status === "failed" && <button type="button" className="button button-outline" onClick={() => void retryNotification(item.id)} disabled={busy}>Retry</button>}</div>)}{!detail.notifications.length && <div className="v6-empty">No application notifications yet.</div>}</div></section>
    <section className="platform-status-panel platform-support-panel"><div className="platform-panel-heading"><div><p className="eyebrow">Support</p><h3>Ask the platform team a question.</h3></div></div><div className="platform-support-layout"><form className="v6-form" onSubmit={submitTicket}><label className="v6-field"><span>Subject</span><input required value={ticket.subject} onChange={(event) => setTicket((current) => ({ ...current, subject: event.target.value }))} placeholder="Domain, import or launch question" /></label><label className="v6-field"><span>Message</span><textarea required value={ticket.message} onChange={(event) => setTicket((current) => ({ ...current, message: event.target.value }))} /></label><button className="button button-dark" disabled={busy}>{busy ? "Sending..." : "Send support request →"}</button></form><div className="platform-ticket-list">{detail.tickets.map((item) => <div key={item.id}><div><strong>{item.subject}</strong><small>{new Date(item.updatedAt).toLocaleString()}</small></div><span className={`platform-status-badge ${item.status}`}>{statusLabel(item.status)}</span><p>{item.adminNote || item.message}</p></div>)}{!detail.tickets.length && <div className="v6-empty">No support requests yet.</div>}</div></div></section>
  </div>;
}
