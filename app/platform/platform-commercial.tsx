...
export type PlatformCommercialSnapshot = {
  ...
  subscription: { id: string; status: string; billingInterval: "monthly" | "annual"; setupFee: number; recurringFee: number; currency: string; nextBillingAt: string | null } | null;
...
export function PlatformCommercialPanel({ applicationId, token, canReview, commercial, onUpdated }: Props) {
  ...
  return <section className="platform-commercial-panel">
    ...
    {commercial.plan && <div className="platform-commercial-actions"><a className="text-link" href="/platform/agreement" target="_blank" rel="noreferrer">Read agreement terms ↗</a>{!commercial.agreement && <button type="button" className="button button-dark" disabled={busy} onClick={() => void act("sign_agreement")}>{busy ? "Saving..." : "Sign and archive agreement →"}</button>}{commercial.subscription?.status === "active" && <button type="button" className="button button-outline" disabled={busy} onClick={() => void act("renew")}>Create renewal invoice</button>}</div>}
