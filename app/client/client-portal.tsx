...
function MerchantWorkspaceTopbar({ siteName, userName, accessLabel }: { siteName: string; userName: string; accessLabel: string }) {
  return <header className="merchant-workspace-topbar"><div className="merchant-workspace-topbar-inner"><button type="button" className="merchant-workspace-store-switcher" aria-label={`当前店铺：${siteName}`}><span className="merchant-workspace-store-icon" aria-hidden="true">▣</span><span>{siteName}</span><span aria-hidden="true">⌄</span></button><Link href="/merchant" className="merchant-workspace-brand"><span className="merchant-workspace-mark">N</span><span><strong>商家工作台</strong><small>店铺经营中心</small></span></Link><div className="merchant-workspace-search" role="search"><span aria-hidden="true">⌕</span><span>搜索商品、SKU、订单等</span><kbd>⌘ K</kbd></div><div className="merchant-workspace-actions"><button type="button" className="merchant-workspace-icon-button" aria-label="查看通知">♧<b>12</b></button><span className="merchant-workspace-user-avatar" aria-hidden="true">{userName.slice(0, 1).toUpperCase()}</span><span className="merchant-workspace-user">{userName}</span><span className="merchant-workspace-role">{accessLabel}</span><a href={"/signout-with-chatgpt?return_to=" + encodeURIComponent("/merchant")}>退出登录</a></div></div></header>;
}
...
export function ClientPortal({ userName, mode = "client" }: { userName: string; mode?: "client" | "merchant" }) {
  ...
  return <main className="client-portal">
    ...
    <div className="client-portal-layout">
      ...
      <div className={section === "products" ? "client-portal-content client-portal-content-products" : "client-portal-content"}>
