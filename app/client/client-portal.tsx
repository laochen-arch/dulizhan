...
import { MerchantTeam, MerchantOrders, MerchantAfterSales, MerchantCustomers } from "../merchant/service-panels";
...
type PortalConfig = { brand?: Record<string, string>; assets?: { hero?: string }; navigation?:Array<{label:string;href:string}>; announcement?:Record<string,string>; content?: { contact?: Record<string, string>; home?:Record<string,string>; policies?:Record<string,string> }; seo?:Record<string,string>; theme?: { colors?: Record<string, string> } };
...
type MerchantAnalytics = { days: number; paidOrders: number; revenue: number; averageOrderValue: number; openAbandonedCheckouts: number; events: Array<{ eventType: string; count: number }> };
type MerchantTask = { id:string; label:string; detail:string; count:number; severity:"urgent"|"warning"|"info"; section:"orders"|"inventory"|"after-sales"|"operations"|"integrations" };
type LaunchCenter = { readiness: { score: number; blockers: Array<{ key: string; label: string; detail: string; source: string }>; launch: { checks: Array<{ key: string; label: string; detail: string; done: boolean; required?: boolean }>; progress: { done: number; total: number } }; health: Array<{ key: string; status: string; detail: string; checkedAt: string }>; openOperations: number }; releases: Array<{ id: string; status: string; label: string; note: string | null; requestedByEmail: string; requestedAt: string; revisionId: string | null; publishedAt: string | null }>; backups?: Array<{ id: string; status: string; reason: string; sizeBytes: number; createdAt: string; verifiedAt: string | null; lastError: string | null }>; tasks?:MerchantTask[]; diff: { totalChanges: number; changes: string[] }; operations: { orders: number; paidOrders: number; openAfterSales: number; lowStock: number; availableUnits: number; failedEvents: number } };
...
const workspacePageCopy: Record<PortalSection, { eyebrow: string; title: string; description: string }> = {
  ...
  customers: { eyebrow: "客户管理", title: "客户管理", description: "统一查看当前店铺的注册客户、访客订单、地址和购买记录。" },
...
export function ClientPortal({ userName, mode = "client" }: { userName: string; mode?: "client" | "merchant" }) {
  ...
  const [storefront,setStorefront]=useState({navigation:"",announcementText:"",announcementAccent:"",heroLabel:"",heroTitleLead:"",heroTitleAccent:"",heroBody:"",heroCta:"",shippingLead:"",deliveryLead:"",returnsLead:"",shippingThreshold:"",seoTitle:"",seoDescription:"",seoKeywords:""});
  ...
  const loadOverview = useCallback(async (signal?: AbortSignal) => {
    ...
    setStorefront({navigation:(config.navigation||[]).map(item=>`${item.label} | ${item.href}`).join("\n"),announcementText:config.announcement?.text||"",announcementAccent:config.announcement?.accent||"",heroLabel:config.content?.home?.heroLabel||"",heroTitleLead:config.content?.home?.heroTitleLead||"",heroTitleAccent:config.content?.home?.heroTitleAccent||"",heroBody:config.content?.home?.heroBody||"",heroCta:config.content?.home?.heroCta||"",shippingLead:config.content?.policies?.shippingLead||"",deliveryLead:config.content?.policies?.deliveryLead||"",returnsLead:config.content?.policies?.returnsLead||"",shippingThreshold:config.content?.policies?.shippingThreshold||"",seoTitle:config.seo?.title||"",seoDescription:config.seo?.description||"",seoKeywords:config.seo?.keywords||""});
  ...
  async function saveBrand(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const navigation=storefront.navigation.split("\n").map(line=>{const [label,...href]=line.split("|");return {label:label?.trim(),href:href.join("|").trim()};}).filter(item=>item.label&&item.href);
      const response = await fetch(`${apiRoot}/brand`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ siteId, brand: { name: brand.name, mark: brand.mark, descriptor: brand.descriptor, tagline: brand.tagline }, hero: brand.hero, contactEmail: brand.contactEmail, tradeEmail: brand.tradeEmail, colors, navigation, announcement:{text:storefront.announcementText,accent:storefront.announcementAccent},home:{heroLabel:storefront.heroLabel,heroTitleLead:storefront.heroTitleLead,heroTitleAccent:storefront.heroTitleAccent,heroBody:storefront.heroBody,heroCta:storefront.heroCta},policies:{shippingLead:storefront.shippingLead,deliveryLead:storefront.deliveryLead,returnsLead:storefront.returnsLead,shippingThreshold:storefront.shippingThreshold},seo:{title:storefront.seoTitle,description:storefront.seoDescription,keywords:storefront.seoKeywords} }) });
  ...
  async function merchantBackup(action: "create" | "verify", backupId?: string) {
    setBusy(true);
    try {
      const response = await fetch("/api/merchant/backups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ siteId, action, backupId }) });
      const payload = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Unable to complete the backup action.");
      await loadOperations();
      setNotice({ tone: "success", text: action === "create" ? "店铺数据已备份并完成校验。" : "无损恢复演练已通过。" });
    } catch (error) { setNotice({ tone: "error", text: error instanceof Error ? error.message : "Unable to complete the backup action." }); }
    finally { setBusy(false); }
  }
  ...
  return <BackofficeShell workspaceRole="merchant" brand={activeSite?.name || "商家工作台"} title={portalSectionLabels[section]} description={activePageCopy.description} current={section} groups={workspaceNavGroups} onNavigate={id => selectSection(id as PortalSection)} user={userName}
    ...
    status={notice && <div className={notice.tone === "error" ? "bo-error" : "bo-info"} role="status">{notice.text}<button type="button" className="text-button" onClick={() => setNotice(null)} aria-label="关闭消息"> ×</button></div>}>
        ...
        {section === "brand" && <form className="client-portal-grid" onSubmit={saveBrand}><article className="client-portal-card"><p className="eyebrow">店铺品牌</p><h2>品牌与首页展示</h2><div className="v6-form-grid"><label className="v6-field"><span>品牌名称</span><input value={brand.name} onChange={(event) => setBrand((current) => ({ ...current, name: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>Logo 标识</span><input value={brand.mark} onChange={(event) => setBrand((current) => ({ ...current, mark: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>品牌标语</span><input value={brand.tagline} onChange={(event) => setBrand((current) => ({ ...current, tagline: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>品牌介绍</span><input value={brand.descriptor} onChange={(event) => setBrand((current) => ({ ...current, descriptor: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>首页大图地址</span><input value={brand.hero} onChange={(event) => setBrand((current) => ({ ...current, hero: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>客服邮箱</span><input type="email" value={brand.contactEmail} onChange={(event) => setBrand((current) => ({ ...current, contactEmail: event.target.value }))} disabled={!canEdit} /></label><label className="v6-field"><span>商务邮箱</span><input type="email" value={brand.tradeEmail} onChange={(event) => setBrand((current) => ({ ...current, tradeEmail: event.target.value }))} disabled={!canEdit} /></label></div><h3>首页首屏与公告</h3><div className="v6-form-grid">{[["announcementText","顶部公告"],["announcementAccent","公告补充"],["heroLabel","首屏小标题"],["heroTitleLead","首屏主标题"],["heroTitleAccent","首屏强调标题"],["heroCta","首屏按钮文字"]].map(([key,label])=><label className="v6-field" key={key}><span>{label}</span><input value={storefront[key as keyof typeof storefront]} onChange={event=>setStorefront(current=>({...current,[key]:event.target.value}))} disabled={!canEdit}/></label>)}</div><label className="v6-field"><span>首屏介绍</span><textarea value={storefront.heroBody} onChange={event=>setStorefront(current=>({...current,heroBody:event.target.value}))} disabled={!canEdit}/></label><label className="v6-field"><span>顶部导航（每行：名称 | /路径，最多 6 项）</span><textarea value={storefront.navigation} onChange={event=>setStorefront(current=>({...current,navigation:event.target.value}))} disabled={!canEdit}/></label><h3>配送与退换货</h3><div className="v6-form-grid">{[["shippingLead","发货说明"],["deliveryLead","配送时效"],["returnsLead","退换货说明"],["shippingThreshold","免邮门槛文案"]].map(([key,label])=><label className="v6-field" key={key}><span>{label}</span><textarea value={storefront[key as keyof typeof storefront]} onChange={event=>setStorefront(current=>({...current,[key]:event.target.value}))} disabled={!canEdit}/></label>)}</div><h3>搜索展示（SEO）</h3><label className="v6-field"><span>页面标题</span><input value={storefront.seoTitle} onChange={event=>setStorefront(current=>({...current,seoTitle:event.target.value}))} disabled={!canEdit}/></label><label className="v6-field"><span>页面描述</span><textarea value={storefront.seoDescription} onChange={event=>setStorefront(current=>({...current,seoDescription:event.target.value}))} disabled={!canEdit}/></label><label className="v6-field"><span>关键词</span><input value={storefront.seoKeywords} onChange={event=>setStorefront(current=>({...current,seoKeywords:event.target.value}))} disabled={!canEdit}/></label><h3>主题颜色</h3><div className="client-color-grid">{["ink", "paper", "warm", "rust", "sage"].map((key) => <label className="v6-field" key={key}><span>{key}</span><input type="text" value={colors[key] || ""} onChange={(event) => setColors((current) => ({ ...current, [key]: event.target.value }))} disabled={!canEdit} /></label>)}</div><button className="button button-dark" disabled={!canEdit || busy}>{busy ? "正在保存…" : "保存店铺草稿"}</button></article><aside className="client-portal-card client-portal-callout"><p className="eyebrow">发布规则</p><h2>保存草稿，审核后发布</h2><p className="v6-muted">商城继续显示已发布内容，草稿通过检查和负责人审核后才更新。</p><div className="client-check-list"><span>✓ 首页、政策和 SEO 一起保存</span><span>✓ 草稿不会覆盖线上商城</span><span>✓ 限时预览链接自动过期</span></div></aside></form>}
        ...
        {section === "customers" && <MerchantCustomers key={siteId} siteId={siteId} onOpenOrders={email=>selectSection("orders",email)}/>}
        ...
        {section === "after-sales" && <MerchantAfterSales siteId={siteId} requests={afterSales} canWrite={capabilities.has("after-sales.write")} onRequests={setAfterSales}/>}
            ...
            {section === "operations" && <section className="v24-launch-shell">{launch && <><div className="v6-card v24-launch-hero"><div><p className="eyebrow">店铺经营概况</p><h2>{launch.readiness.score}% 发布准备度。</h2><p className="v6-muted">先处理缺失资料，再提交发布审核，草稿修改不会直接覆盖已上线商城。</p></div><div className="v24-score"><strong>{launch.operations.availableUnits}</strong><span>可售库存</span></div></div><div className="v24-metrics"><div><span>全部订单</span><strong>{launch.operations.orders}</strong></div><div><span>已付款</span><strong>{launch.operations.paidOrders}</strong></div><div><span>待处理售后</span><strong>{launch.operations.openAfterSales}</strong></div><div><span>低库存</span><strong>{launch.operations.lowStock}</strong></div><div><span>系统异常</span><strong>{launch.operations.failedEvents}</strong></div></div><div className="v6-grid"><article className="v6-card"><p className="eyebrow">今日待办</p><h3>{launch.tasks?.reduce((sum,item)=>sum+item.count,0)||0} 项业务待处理。</h3><div className="v24-list">{launch.tasks?.map(item=><button type="button" className="bo-task-row" key={item.id} onClick={()=>selectSection(item.section)}><span className={`v24-dot ${item.severity==="urgent"?"error":""}`}>{item.count}</span><span><strong>{item.label}</strong><small>{item.detail}</small></span><b>去处理 →</b></button>)}{!launch.tasks?.length&&<div className="v6-empty">当前没有业务待办。</div>}</div></article><article className="v6-card"><p className="eyebrow">发布申请</p><h3>提交店铺发布审核</h3><label className="v6-field"><span>变更说明</span><textarea value={releaseNote} onChange={(event) => setReleaseNote(event.target.value)} disabled={!canEdit} placeholder="说明本次商品、装修或营销变更" /></label><button type="button" className="button button-dark" onClick={() => void requestRelease()} disabled={!canEdit || busy || !launch.diff.totalChanges}>提交审核 →</button><div className="v24-release-list">{launch.releases.slice(0, 5).map((release) => <div key={release.id}><div><strong>{release.status}</strong><small>{release.label} · {new Date(release.requestedAt).toLocaleString()}</small></div></div>)}</div></article></div>{launch.readiness.blockers.length>0&&<article className="v6-card"><p className="eyebrow">上线前还需完成</p><div className="v24-list">{launch.readiness.blockers.map(item=><div key={item.key}><span className="v24-dot error">!</span><span><strong>{item.label}</strong><small>{item.detail}</small></span></div>)}</div></article>}</>}</section>}

            {section === "operations" && mode === "merchant" && capabilities.has("merchant.data.export") && launch && <section className="client-portal-card"><div className="v6-card-heading"><div><p className="eyebrow">数据备份与恢复</p><h2>保留可验证的店铺数据副本</h2></div><button type="button" className="button button-dark" onClick={() => void merchantBackup("create")} disabled={busy}>创建备份</button></div><p className="v6-muted">备份保存在私有存储中，不包含支付和邮件密钥。恢复演练只校验可恢复性，不会改动线上数据。</p><div className="v24-release-list">{(launch.backups || []).map((backup) => <div key={backup.id}><div><strong>{backup.status === "verified" ? "已验证备份" : "备份需要处理"}</strong><small>{new Date(backup.createdAt).toLocaleString()} · {Math.ceil(backup.sizeBytes / 1024)} KB</small>{backup.lastError && <p>{backup.lastError}</p>}</div><div className="v6-actions"><button type="button" className="button button-outline" onClick={() => void merchantBackup("verify", backup.id)} disabled={busy}>恢复演练</button>{backup.status === "verified" && <a className="text-link" href={`/api/merchant/backups?siteId=${encodeURIComponent(siteId)}&download=${encodeURIComponent(backup.id)}`}>下载导出 →</a>}</div></div>)}{!(launch.backups || []).length && <div className="v6-empty">还没有备份。正式上线前请创建并完成恢复演练。</div>}</div></section>}
        ...
        {section === "team" && <MerchantTeam siteId={siteId} members={teamMembers} canWrite={canTeam} onMembers={setTeamMembers}/>}
            {section === "operations" && mode === "merchant" && launch?.analytics && <section className="client-portal-card"><div className="v6-card-heading"><div><p className="eyebrow">近 {launch.analytics.days} 天</p><h2>经营数据概览</h2></div></div><div className="v24-metrics"><div><span>销售净额</span><strong>{formatMoney(launch.analytics.revenue, "usd")}</strong></div><div><span>已付款订单</span><strong>{launch.analytics.paidOrders}</strong></div><div><span>平均客单价</span><strong>{formatMoney(launch.analytics.averageOrderValue, "usd")}</strong></div><div><span>未完成结账</span><strong>{launch.analytics.openAbandonedCheckouts}</strong></div>{launch.analytics.events.slice(0, 2).map((event) => <div key={event.eventType}><span>{({page_view:"访问",product_view:"商品浏览",add_to_cart:"加入购物车",checkout_started:"开始结账",purchase:"购买",search:"搜索",wishlist_add:"收藏"} as Record<string,string>)[event.eventType]||event.eventType}</span><strong>{event.count}</strong></div>)}</div></section>}
    </>}
  </BackofficeShell>;
}
