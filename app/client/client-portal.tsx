...
export function ClientPortal({ userName, mode = "client" }: { userName: string; mode?: "client" | "merchant" }) {
  ...
  const [section, setSection] = useState<"brand" | "products" | "campaigns" | "team" | "orders" | "after-sales" | "operations" | "integrations">("brand");
  ...
  const capabilities = new Set(overview?.capabilities || []);
  ...
  return <main className="client-portal">
    <header className="client-portal-header"><div><p className="eyebrow">{mode === "merchant" ? "Merchant workspace / Store operations" : "Northline CMS / Client portal"}</p><h1>{mode === "merchant" ? `Operate ${activeSite?.name || overview.siteId}.` : `Run ${activeSite?.name || overview.siteId} yourself.`}</h1><p className="v6-muted">Signed in as {userName}. Products, inventory, orders and storefront changes stay isolated to this merchant site.</p></div><div className="client-portal-actions"><button type="button" className="button button-outline" onClick={() => void copyPreviewShare()} disabled={!canEdit || busy}>Copy draft link</button><a className="button button-dark" href={`/admin?tab=v24&siteId=${encodeURIComponent(siteId)}`}>Open platform launch review →</a></div></header>
    ...
    <nav className="client-portal-tabs" aria-label="Merchant workspace sections">{(["brand", "products", ...(mode === "merchant" ? ["campaigns"] : []), ...(mode === "merchant" && canTeam ? ["team"] : []), "orders", "after-sales", "operations", "integrations"] as const).map((item) => <button type="button" key={item} className={section === item ? "is-active" : ""} onClick={() => setSection(item)}>{item === "brand" ? "Storefront setup" : item === "products" ? "Products & stock" : item === "campaigns" ? "Marketing" : item === "team" ? "Team access" : item === "orders" ? "Orders" : item === "after-sales" ? "After-sales" : item === "operations" ? "Launch & analytics" : "Payments & email"}</button>)}</nav>
