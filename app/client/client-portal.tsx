...
export type MerchantTeamMember = { siteId: string; userId: string; email: string; role: "merchant_owner" | "merchant_manager" | "merchant_staff"; source: string; createdAt: string; updatedAt: string };
...
export function ClientPortal({ userName, mode = "client" }: { userName: string; mode?: "client" | "merchant" }) {
  ...
  const merchantRoleLabel = overview?.merchantRole === "merchant_owner" ? "店铺所有者" : overview?.merchantRole === "merchant_manager" ? "店铺管理员" : overview?.merchantRole === "merchant_staff" ? "订单与售后工作人员" : overview?.role || "Workspace";
