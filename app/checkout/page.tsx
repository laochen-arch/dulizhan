...
export default function CheckoutPage() {
  ...
  const [paymentState, setPaymentState] = useState<"checking" | "paid" | "pending" | "cancelled">("checking");
  ...
  useEffect(() => {
    ...
    if (!paidSession || statusStarted.current) return;
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (active && payload.status) {
          ...
          if (payload.status.paymentStatus === "paid") return setPaymentState("paid");
      ...
      if (active && attempts < 6) timer = window.setTimeout(() => void poll(), 1500);
  ...
  if (submitted || paidSession || cancelled) return <div className="empty-state container section-pad"><span className="empty-mark">{visiblePaymentState === "paid" ? "OK" : "…"}</span><p className="eyebrow">{visiblePaymentState === "paid" ? "Payment confirmed" : visiblePaymentState === "cancelled" ? "Payment canceled" : "Payment received"}</p><h1>{visiblePaymentState === "cancelled" ? "Your bag is still here." : "See you out there."}</h1><p>{visiblePaymentState === "paid" ? `Order ${orderNumber || "confirmed"}. A receipt and order update will be sent to your email.` : visiblePaymentState === "cancelled" ? "No charge was made. Your bag is still saved so you can try again." : "Stripe has returned you to the storefront. We are confirming your payment and will update the order shortly."}</p><Link href={visiblePaymentState === "cancelled" ? "/cart" : "/shop"} className="button button-dark">{visiblePaymentState === "cancelled" ? "Return to bag -&gt;" : "Continue shopping -&gt;"}</Link></div>;
