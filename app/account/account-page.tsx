...
export function AccountPage() {
  ...
  return <main className="account-shell container">
    ...
    {tab === "orders" && <section className="account-panel"><div className="account-panel-heading"><div><p className="eyebrow">Order history</p><h2>Your orders.</h2></div><a className="text-link" href="/shop">Shop the collection →</a></div>{orders.length ? <div className="account-order-list">{orders.map((order) => <div className="account-order-row" key={order.id}><a href={`/account/orders/${encodeURIComponent(order.id)}`}><span><strong>{order.orderNumber}</strong><small>{new Date(order.createdAt).toLocaleDateString()} · {order.paymentStatus}</small></span><span><strong>{formatMoney(order.total, order.currency)}</strong><small>{order.trackingNumber ? `Tracking ${order.trackingNumber}` : order.fulfillmentStatus}</small></span><span aria-hidden="true">↗</span></a><ReorderButton orderId={order.id} /></div>)}</div> : <div className="account-empty"><h3>No account orders yet.</h3><p>Orders placed with this email will appear here after payment confirmation.</p></div>}</section>}
