...
export function AccountPage() {
  ...
  if (!access.authenticated) return <main className="account-shell container"><section className="account-signin"><p className="eyebrow">Northline account</p><h1>Keep your orders<br /><em>in one place.</em></h1><p>Sign in with your email account or the secure workspace identity to view orders, delivery status and saved addresses.</p><div className="v6-actions"><a className="button button-dark" href="/auth/login?return_to=%2Faccount">Sign in to continue with email <span>→</span></a><a className="button button-outline" href="/signin-with-chatgpt?return_to=%2Faccount">Use ChatGPT sign-in</a></div><p className="account-note">Guest checkout and order lookup remain available from <a href="/orders">Track order</a>.</p></section></main>;

  return <main className="account-shell container">
    <header className="account-header"><div><p className="eyebrow">{access.site.name} / Account</p><h1>Welcome back,<br /><em>{profile?.displayName || access.user?.displayName || "traveler"}.</em></h1></div><a className="text-link" href="/api/auth/email/logout?return_to=%2F">Sign out →</a></header>
