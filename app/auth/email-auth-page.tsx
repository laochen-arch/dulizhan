...
export function EmailAuthPage({ mode }: { mode: "login" | "register" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({})) as { error?: string; verificationSent?: boolean };
      if (!response.ok) throw new Error(payload.error || "Unable to continue.");
      if (mode === "register") { setMessage(payload.verificationSent ? "账号已创建，验证邮件已发送。你也可以先进入门户继续配置。" : "账号已创建，当前环境未配置验证邮件服务。你可以直接进入门户。"); window.setTimeout(() => window.location.assign(returnTo), 900); }
