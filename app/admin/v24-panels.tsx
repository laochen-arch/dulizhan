...
export function V24OperationsPanel({ activeSiteId, cmsRole, onNotice }: { activeSiteId: string; cmsRole: CmsRole; onNotice: (notice: Notice) => void }) {
  ...
  return <TaskSections className="v24-launch-shell" labels={["检查概览","经营摘要","检查结果","预览与发布申请","审核队列","回滚记录"]}>
    <div className="v6-card v24-launch-hero"><div><p className="eyebrow">V24 / Production launch center</p><h2>Ship each tenant with a traceable release gate.</h2><p className="v6-muted">Health checks, provider status, preview sharing, owner approval and rollback are stored per client site.</p></div><div className="v24-score"><strong>{center.readiness.score}%</strong><span>ready</span><button type="button" className="button button-dark" onClick={() => void runHealthChecks()} disabled={!canEdit || busy}>{busy ? "Working..." : "Run health checks →"}</button></div></div>
