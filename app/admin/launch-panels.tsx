...
export function LaunchSetupPanel({ activeSiteId, commerceConfiguration, domains, onboarding, busy, onRefresh, onNotice }: {
  ...
  const environmentKeys = commerceConfiguration?.environmentKeys || ["CMS_SECRETS_KEY"];
  ...
  async function runCoreRecovery() {
    try {
      const response = await fetch("/api/cms/commerce/ops", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ siteId: activeSiteId }) });
      const result = await response.json().catch(() => ({})) as { expired?: number; notifications?: { retried?: number }; paymentEvents?: { retried?: number; failed?: number }; orders?: { paid?: number; failed?: number; errors?: number } | null; refunds?: { completed?: number; failed?: number; errors?: number } | null; errors?: string[]; error?: string };
      if (!response.ok) throw new Error(result.error || "Unable to run commerce recovery.");
      const warning = result.errors?.length ? ` ${result.errors.length} step(s) need attention.` : "";
      onNotice({ tone: result.errors?.length ? "info" : "success", text: `Recovery complete: ${result.expired || 0} reservations, ${result.notifications?.retried || 0} emails, ${result.paymentEvents?.retried || 0} webhook retries, ${result.refunds?.completed || 0} refunds reconciled.${warning}` });
      await onRefresh();
    } catch (error) {
      onNotice({ tone: "error", text: error instanceof Error ? error.message : "Unable to run commerce recovery." });
    }
  }
  ...
  return <TaskSections className="v13-setup-stack" labels={["配置检查","支付与邮件","回调与域名","失败重试与验收","平台加密配置"]}>
    ...
    <div className="v13-provider-grid"><article className="v6-card"><div className="v6-card-heading"><div><p className="eyebrow">Operations recovery</p><h3>Release and retry queue</h3></div><span>Manual safety controls</span></div><p className="v6-muted">The same recovery pass used by the scheduled worker releases stock, retries Resend and PayPal events, reconciles payments, and finishes pending refunds.</p><div className="v6-actions"><button className="button button-dark" onClick={() => void runCoreRecovery()} disabled={busy}>Run core recovery</button><button className="button button-outline" onClick={() => void releaseExpired()} disabled={busy}>Release expired inventory</button><button className="button button-outline" onClick={() => void retryDueEmails()} disabled={busy}>Retry due emails</button></div></article><article className="v6-card"><div className="v6-card-heading"><div><p className="eyebrow">Production test checklist</p><h3>Record the go-live evidence.</h3></div><span>{productionTests.filter((item) => testChecklist[item.key]).length}/{productionTests.length}</span></div><p className="v6-help">These checks are saved to the selected client site and are required before its first formal publish.</p><div className="v13-checklist">{productionTests.map((item) => <label className={testChecklist[item.key] ? "done" : ""} key={item.key}><input type="checkbox" checked={Boolean(testChecklist[item.key])} onChange={(event) => void updateTestCheck(item.key, event.target.checked)} /><span>{testChecklist[item.key] ? "OK" : "!"}</span><div><strong>{item.label}</strong><small>Complete this step after the matching provider or storefront action.</small></div></label>)}</div></article></div>
    <div className="v6-card v13-env-card"><div className="v6-card-heading"><div><p className="eyebrow">Runtime variables</p><h3>Configure the encryption key in Sites</h3></div><button className="text-button" onClick={() => copyText(environmentKeys.join("\n"), onNotice)}>Copy names</button></div><div className="v13-env-list">{environmentKeys.map((key) => <code key={key}>{key}</code>)}</div><p className="v6-help">Provider secrets are now entered per tenant in V23 configuration and encrypted before storage. Set CMS_SECRETS_KEY once in the Sites production environment, then run the provider checks again.</p><button className="text-button" onClick={() => void onRefresh()}>Refresh configuration status</button></div>
  </TaskSections>;
}
