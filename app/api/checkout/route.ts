...
export async function POST(request: Request) {
  try {
    ...
    const messages: Record<string, string> = { PAYMENT_NOT_CONFIGURED: "Payments are not configured for this storefront yet.", CMS_SECRETS_NOT_CONFIGURED: "The storefront payment configuration needs CMS_SECRETS_KEY before checkout can start.", CMS_SECRETS_INVALID: "The storefront payment configuration cannot be decrypted. Re-save the provider credentials with the current CMS_SECRETS_KEY.", STOCK_UNAVAILABLE: "One or more items are no longer available in the requested quantity.", PRODUCT_UNAVAILABLE: "One or more products are no longer available.", INVALID_CHECKOUT: "Check the checkout fields and cart items before continuing.", PAYMENT_PROVIDER_ERROR: "PayPal rejected the checkout request. Check the Sandbox/Live mode and PayPal app credentials." };
