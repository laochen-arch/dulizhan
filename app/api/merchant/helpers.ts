...
export function merchantErrorResponse(error: unknown) {
  ...
  const notFound = ["SITE_NOT_FOUND", "PRODUCT_NOT_FOUND", "ORDER_NOT_FOUND", "CUSTOMER_NOT_FOUND", "AFTER_SALES_NOT_FOUND", "APPLICATION_NOT_FOUND"];
  const invalid = ["INVALID_PRODUCT", "INVALID_IMPORT", "INVALID_INVENTORY", "INVENTORY_BELOW_RESERVED", "INVALID_ORDER_STATUS", "ORDER_NOT_PAID", "ORDER_CANCEL_REQUIRES_REFUND", "INVALID_AFTER_SALES", "INVALID_COUPON", "INVALID_BUNDLE", "INVALID_BRAND", "INVALID_INTEGRATION", "INVALID_MEMBER", "VIEWER_READ_ONLY", "LAST_OWNER", "CANNOT_REMOVE_SELF", "PRODUCT_IN_USE"];
  ...
  if (message === "BACKUP_NOT_FOUND") return Response.json({ error: "The requested backup was not found.", code: message }, { status: 404, headers: { "Cache-Control": "private, no-store" } });
  if (["BACKUP_VALIDATION_FAILED", "BACKUP_OBJECT_NOT_FOUND", "BACKUP_OBJECT_UNREADABLE"].includes(message)) return Response.json({ error: "The backup restore drill failed. Create a fresh backup before launch.", code: message }, { status: 409, headers: { "Cache-Control": "private, no-store" } });
