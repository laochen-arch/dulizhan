...
export async function POST(request: Request) {
  try {
    const payload = await request.json() as { siteId?: string; orderId?: string; amount?: number; reason?: string; restockItems?: Array<{ productId: string; variantId: string; quantity: number }>; idempotencyKey?: string };
    ...
    // Inventory is restored only for the explicitly selected items after PayPal confirms the refund.
    const idempotencyKey=request.headers.get("x-idempotency-key")||payload.idempotencyKey||"";
    const detail=await createRefund(access.site.id,payload.orderId,payload.amount,payload.reason,payload.restockItems||[],access.user!.userId,access.user!.email,idempotencyKey);
