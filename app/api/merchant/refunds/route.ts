...
export async function POST(request: Request) {
  try {
    const payload = await request.json() as { siteId?: string; orderId?: string; amount?: number; reason?: string };
    ...
    const detail=await createRefund(access.site.id,payload.orderId,payload.amount,payload.reason,[],access.user!.userId,access.user!.email);
