...
export function accountErrorResponse(error: unknown) {
  ...
  const status = message === "AUTH_REQUIRED" ? 401 : ["ORDER_NOT_FOUND", "ADDRESS_NOT_FOUND", "PRODUCT_NOT_FOUND"].includes(message) ? 404 : ["INVALID_PROFILE", "INVALID_ADDRESS", "INVALID_CART"].includes(message) ? 400 : 500;
