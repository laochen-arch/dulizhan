...
export function errorResponse(error: unknown) {
  ...
  if (["REVISION_NOT_FOUND", "ASSET_NOT_FOUND", "OPERATION_EVENT_NOT_FOUND", "RELEASE_REQUEST_NOT_FOUND", "BACKUP_NOT_FOUND"].includes(message)) return Response.json({ error: "The requested CMS record was not found.", code: message }, { status: 404, headers: { "Cache-Control": "no-store" } });
  if (["BACKUP_VALIDATION_FAILED", "BACKUP_OBJECT_NOT_FOUND", "BACKUP_OBJECT_UNREADABLE"].includes(message)) return Response.json({ error: "The backup restore drill failed. Keep the failed record and create a fresh backup before launch.", code: message }, { status: 409, headers: { "Cache-Control": "no-store" } });
  ...
  if (["LAST_OWNER", "CANNOT_REMOVE_SELF", "DOMAIN_IN_USE", "OWNER_APPROVAL_REQUIRED", "RELEASE_NOT_APPROVED", "INVALID_RELEASE_ACTION"].includes(message)) return Response.json({ error: message === "OWNER_APPROVAL_REQUIRED" ? "Only the site owner can approve, publish or roll back a release." : "This release action is not allowed in the current state.", code: message }, { status: message === "OWNER_APPROVAL_REQUIRED" ? 403 : 400, headers: { "Cache-Control": "no-store" } });
  if (["INVALID_SITE", "INVALID_MEMBER", "INVALID_SCHEDULE", "INVALID_LAUNCH_CHECK", "INVALID_DELIVERY_STEP", "INVALID_DELIVERY_STATUS", "INVALID_OPERATION", "INVALID_INTEGRATION", "CMS_SECRETS_NOT_CONFIGURED", "CMS_SECRETS_INVALID", "CMS_OLD_SECRETS_NOT_CONFIGURED"].includes(message)) return Response.json({ error: message === "CMS_SECRETS_NOT_CONFIGURED" ? "Configure CMS_SECRETS_KEY (a 32+ character production secret) before saving tenant credentials." : message === "CMS_SECRETS_INVALID" ? "The tenant credentials cannot be decrypted with the current CMS_SECRETS_KEY." : message === "CMS_OLD_SECRETS_NOT_CONFIGURED" ? "Provide the previous CMS_SECRETS_KEY (32+ characters) to migrate tenant credentials." : "The submitted CMS fields are invalid.", code: message }, { status: message === "CMS_SECRETS_NOT_CONFIGURED" || message === "CMS_SECRETS_INVALID" ? 503 : 400, headers: { "Cache-Control": "no-store" } });
  ...
  if (message.startsWith("INVALID_PRODUCT:")) {
    try {
      return Response.json({ error: "The product has invalid commerce fields.", code: "INVALID_PRODUCT", errors: JSON.parse(message.slice("INVALID_PRODUCT:".length)) }, { status: 400, headers: { "Cache-Control": "no-store" } });
    } catch {
      return Response.json({ error: "The product has invalid commerce fields.", code: "INVALID_PRODUCT" }, { status: 400, headers: { "Cache-Control": "no-store" } });
    }
  }
  if (["INVALID_INVENTORY", "INVENTORY_BELOW_RESERVED", "INVALID_ORDER_STATUS", "ORDER_NOT_PAID", "ORDER_NOT_REFUNDABLE", "INVALID_REFUND_AMOUNT", "INVALID_REFUND_RESTOCK", "REFUND_PAYMENT_NOT_FOUND", "REFUND_IDEMPOTENCY_REQUIRED", "REFUND_IDEMPOTENCY_CONFLICT", "PAYMENT_NOT_CONFIGURED", "REFUND_PROVIDER_ERROR", "INVALID_AFTER_SALES", "INVALID_COUPON", "COUPON_UNAVAILABLE", "INVALID_REVIEW", "INVALID_ANALYTICS_EVENT"].includes(message)) return Response.json({ error: message === "ORDER_NOT_PAID" ? "Paid orders must be confirmed before fulfillment can advance." : message === "PAYMENT_NOT_CONFIGURED" ? "PayPal is not configured in the production runtime." : message === "REFUND_PROVIDER_ERROR" ? "PayPal could not complete the refund. Review the refund record and retry from the provider if needed." : message === "REFUND_IDEMPOTENCY_REQUIRED" ? "A unique refund request key is required." : message === "REFUND_IDEMPOTENCY_CONFLICT" ? "This refund request key was already used for another order." : "The submitted commerce fields are invalid.", code: message }, { status: message === "REFUND_PROVIDER_ERROR" ? 502 : message === "REFUND_IDEMPOTENCY_CONFLICT" ? 409 : 400, headers: { "Cache-Control": "no-store" } });
