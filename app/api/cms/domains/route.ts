...
export async function POST(request: Request) {
  try {
    ...
    const database = getCmsDatabase();
    await ensureCmsSchema(database);
    const domain: CmsDomain = { id: `domain_${crypto.randomUUID()}`, siteId, hostname, status: "pending", verificationToken: `verify_${crypto.randomUUID()}`, verifiedAt: null, lastCheckedAt: null, dnsTarget: null, sslStatus: "pending", lastError: null, createdAt: new Date().toISOString() };
    await database.prepare(`INSERT INTO cms_site_domains (id, site_id, hostname, status, verification_token, verified_at, last_checked_at, created_at)
      VALUES (?1, ?2, ?3, 'pending', ?4, NULL, NULL, ?5) ON CONFLICT(hostname) DO NOTHING`).bind(domain.id, siteId, hostname, domain.verificationToken, domain.createdAt).run();
    const saved = await database.prepare(`SELECT id, site_id AS siteId, hostname, status, verification_token AS verificationToken,
      verified_at AS verifiedAt, last_checked_at AS lastCheckedAt, dns_target AS dnsTarget, ssl_status AS sslStatus,
      last_error AS lastError, created_at AS createdAt FROM cms_site_domains WHERE hostname = ?1`).bind(hostname).first<CmsDomain>();
    if (!saved || saved.siteId !== siteId) return Response.json({ error: "此域名已绑定其他站点，请先核实归属。", code: "DOMAIN_ALREADY_BOUND" }, { status: 409 });
    await database.prepare("UPDATE cms_sites SET domain = ?1, updated_at = ?2 WHERE id = ?3").bind(hostname, domain.createdAt, siteId).run();
    return Response.json({ domain: saved }, { status: saved.id === domain.id ? 201 : 200, headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    return errorResponse(error);
  }
}
