...
export async function POST(request: Request) {
  try {
    const payload = await request.json() as { siteId?: string; orderId?: string; amount?: number; reason?: string; restockItems?: Array<{ productId: string; variantId: string; quantity: number }> };
    ...
    const detail = await createRefund(siteId, payload.orderId, payload.amount, payload.reason || "", payload.restockItems, access.user.userId, access.user.email);
