...
export default function Home() {
  ...
  return <div className="storefront-appstore appstore-home">
    {modules.has("hero") && <section className="appstore-hero-section container">
      ...
      <div className="appstore-hero-card">
        <img className="appstore-hero-image" src={config.assets.hero} alt="" fetchPriority="high" decoding="async" />
    ...
    {modules.has("story") && <section className="appstore-feature-section container section-pad">
      <div className="appstore-feature-grid">
        <div className="appstore-feature-image"><img src={config.assets.story} alt="Mist moving through a mountain valley" loading="lazy" decoding="async" /></div>
    ...
    {modules.has("journal") && <section className="appstore-editorial-section section-pad"><div className="container"><div className="appstore-section-heading compact"><div><p className="appstore-kicker">{config.content.home.journalLabel}</p><h2>{config.content.home.journalTitleLead}<br /><em>{config.content.home.journalTitleAccent}</em></h2></div><Link href="/faq" className="appstore-text-link">More notes <span>↗</span></Link></div><div className="appstore-editorial-grid"><Link href="/shipping" className="appstore-editorial-card appstore-editorial-card-large"><img src={config.assets.journalHero} alt="Hiker looking across a mountain landscape" loading="lazy" decoding="async" /><div><span>Field notes / 06.12.24</span><h3>How to pack for the version of a trip you can&apos;t predict.</h3><span className="appstore-text-link">Read article <span>↗</span></span></div></Link><div className="appstore-editorial-list"><Link href="/faq" className="appstore-editorial-row"><span>01</span><div><small>Good questions</small><strong>What makes a piece worth carrying?</strong></div><span>↗</span></Link><Link href="/shipping" className="appstore-editorial-row"><span>02</span><div><small>On the way</small><strong>Our approach to less wasteful shipping.</strong></div><span>↗</span></Link><Link href="/about" className="appstore-editorial-row"><span>03</span><div><small>From the archive</small><strong>A field guide to the {config.brand.name} color palette.</strong></div><span>↗</span></Link></div></div></div></section>}
